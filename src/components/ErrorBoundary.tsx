      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-8">
          <div className="max-w-lg w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 text-center">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">系统异常</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">页面渲染遇到错误，请尝试刷新或联系管理员</p>
            <div className="flex gap-3 justify-center">
              ...
              <button onClick={() => window.location.reload()} className="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-sm font-medium hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">刷新页面</button>
            </div>
            {this.state.error && (
              <div className="mt-6 text-left">
                <button onClick={() => this.setState(s => ({ ...s, showDetails: !s.showDetails }))} className="text-xs text-slate-400 hover:text-slate-600 underline">
                  {this.state.showDetails ? '收起详情' : '展开错误详情'}
                </button>
                {this.state.showDetails && (
                  <pre className="mt-2 p-3 bg-slate-100 dark:bg-slate-900 rounded text-xs text-red-600 dark:text-red-400 overflow-auto max-h-40">
