...
const trustSteps: TrustStep[] = [
  {
    ...
    status: 'completed', color: 'text-brand-600', bgColor: 'bg-brand-50'
  },
  {
...
export default function TrustChainVisualizer({ compact = false, onStepClick }: Props) {
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-brand-50 to-trust-50 rounded-xl border border-brand-100/50">
        <Shield className="w-4 h-4 text-brand-600" />
        <span className="text-xs font-semibold text-brand-700">信任传导链路</span>
        <div className="flex items-center gap-0.5 ml-2">
          {trustSteps.map((step, i) => (
            <div key={step.id} className="flex items-center">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center ${step.status === 'completed' ? 'bg-brand-500' : step.status === 'active' ? 'bg-amber-400' : 'bg-ink-200'}`}>
                <step.icon className="w-3 h-3 text-white" />
              </div>
              {i < trustSteps.length - 1 && (
                <div className={`w-4 h-0.5 ${i < 2 ? 'bg-brand-300' : 'bg-ink-200'}`} />
              )}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="bg-surface rounded-2xl border border-border/80 shadow-card p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-base font-bold text-foreground">信任传导链路</h3>
          <p className="text-xs text-muted">CA信任 → 认证企业 → 真实岗位 → 候选人 · 五步链路完整可演示</p>
        </div>
      ...
      <div className="relative">
        {/* 连接线 */}
        <div className="absolute top-10 left-0 right-0 h-0.5 bg-gradient-to-r from-brand-200 via-primary-300 to-trust-200 mx-16 z-0" />
          ...
          {trustSteps.map((step, index) => (
            ...
            >
              <div className={`w-16 h-16 rounded-2xl ${step.bgColor} border-2 ${
                hoveredStep === step.id ? 'border-brand-400 shadow-glass-hover scale-105' : 'border-transparent shadow-card'
              ...
              {hoveredStep === step.id && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="absolute top-full mt-2 w-48 p-3 bg-surface rounded-xl border border-border shadow-elevated z-50"
                >
      ...
      <div className="mt-6 pt-4 border-t border-border grid grid-cols-3 gap-4">
        <div className="text-center">
          <p className="text-lg font-bold text-brand-600">SM2/SM3</p>
          <p className="text-[10px] text-muted">国密签名算法</p>
        </div>
        <div className="text-center">
          <p className="text-lg font-bold text-trust-600">单一接口</p>
          <p className="text-[10px] text-muted">CA校验收敛设计</p>
        </div>
        <div className="text-center">
          <p className="text-lg font-bold text-foreground">零信任</p>
          <p className="text-[10px] text-muted">越权访问一律拒绝</p>
        </div>
      </div>
    </div>
  );
