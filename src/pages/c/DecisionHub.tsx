...
const typeConfig = {
  offer_compare: { label: '选Offer', icon: ArrowRightLeft, color: 'text-primary-600 bg-primary-50 border-primary-100' },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">求职决策社区</h1>
          <p className="text-sm text-slate-500 mt-1">真实数据辅助决策 · 同行经验陪伴 · 不再孤立做选择</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
            activeType === 'all' ? 'bg-primary-600 text-white shadow-sm' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'
          }`}
        >
          全部话题
        </button>
        {Object.entries(typeConfig).map(([key, config]) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all flex items-center gap-2 ${
              activeType === key ? 'bg-primary-600 text-white shadow-sm' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'
            }`}
          >
        ...
        {filteredThreads.map((thread, index) => {
          ...
          return (
            <motion.div
              ...
              className="bg-white rounded-xl border border-slate-200/80 shadow-card p-5 hover:shadow-card-hover transition-all cursor-pointer"
            >
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-600 text-sm font-bold flex-shrink-0">
                  {thread.avatar}
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-slate-400 ml-auto">{thread.timestamp}</span>
                  </div>

                  <h3 className="text-base font-bold text-slate-900">{thread.title}</h3>
                  <p className="text-sm text-slate-600 mt-2 line-clamp-2">{thread.content}</p>
                  ...
                  {thread.dataPoints.length > 0 && (
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      {thread.dataPoints.map((dp, i) => (
                        <span key={i} className="px-2 py-0.5 bg-primary-50 rounded text-[10px] font-medium text-primary-700 border border-primary-100">
                          {dp}
                        </span>
                      ))}
                    </div>
