  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">继任沙盘</h1>
          <p className="text-sm text-slate-500 mt-1">Succession Planning · 关键岗位继任梯队管理与推演</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="stat-card px-4 py-2"><p className="text-[10px] text-slate-400">高风险岗位</p><p className="text-lg font-bold text-error-600">{totalRisk}</p></div>
          <div className="stat-card px-4 py-2"><p className="text-[10px] text-slate-400">平均就绪度</p><p className="text-lg font-bold text-primary-600">{avgReadiness}%</p></div>
          <button onClick={() => setWhatIfMode(!whatIfMode)} className={`btn-secondary ${whatIfMode ? 'ring-2 ring-primary-200' : ''}`}>
      ...
      {/* What-If Mode Banner */}
      {whatIfMode && (
        <div className="mb-6 p-4 bg-primary-50 rounded-xl border border-primary-100 flex items-center gap-3 animate-in">
          <Zap className="w-5 h-5 text-primary-600" />
          <div className="flex-1">
            <p className="text-sm font-medium text-primary-800">What-If 推演模式已开启</p>
            <p className="text-xs text-primary-600">拖动继任者就绪度滑块，模拟不同培养投入下的梯队变化，然后点击"启动推演"查看结果</p>
          </div>
        </div>
      )}
            ...
            <div key={pos.id} className={`glass-card overflow-hidden transition-all ${pos.riskLevel === 'high' ? 'border-l-4 border-l-error-400' : pos.riskLevel === 'medium' ? 'border-l-4 border-l-warning-400' : 'border-l-4 border-l-success-400'}`}>
              <div className="p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-base font-semibold text-slate-800">{pos.position}</h3>
                ...
                <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl mb-4">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-sm font-bold">{pos.incumbent.name[0]}</div>
                  <div className="flex-1">
                    <p className="text-sm font-medium text-slate-800">{pos.incumbent.name}</p>
                    <p className="text-xs text-slate-500">在任 {pos.incumbent.tenure}年</p>
                  </div>
                ...
                <div className="space-y-2">
                  {pos.successors.map((s, i) => {
                    ...
                    return (
                      <div key={i} className="p-2.5 rounded-lg hover:bg-slate-50 transition-colors">
                        <div className="flex items-center gap-3">
                          <span className={`w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center ${i === 0 ? 'bg-primary-500 text-white' : 'bg-primary-100 text-primary-600'}`}>{i + 1}</span>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-slate-700 truncate">{s.name}</p>
                            <p className="text-[10px] text-slate-400">{s.currentRole}</p>
                          </div>
                          <div className="text-right">
                            <p className={`text-xs font-bold ${displayReadiness >= 80 ? 'text-success-600' : displayReadiness >= 60 ? 'text-primary-600' : 'text-warning-600'}`}>
                              {displayReadiness}%
                        ...
                        <div className="mt-1.5 ml-8">
                          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full transition-all duration-500 ${displayReadiness >= 80 ? 'bg-success-500' : displayReadiness >= 60 ? 'bg-primary-500' : 'bg-warning-500'}`} style={{ width: `${Math.min(100, displayReadiness)}%` }} />
                          </div>
                        </div>
                        ...
                        {whatIfMode && !simResults && (
                          <div className="mt-2 ml-8 flex items-center gap-2">
                            <span className="text-[10px] text-slate-400">调整:</span>
                            ...
                            <span className={`text-[10px] font-medium ${adjustment > 0 ? 'text-emerald-600' : adjustment < 0 ? 'text-red-600' : 'text-slate-400'}`}>{adjustment > 0 ? '+' : ''}{adjustment}</span>
                          </div>
                        )}
                        ...
                        {isExpanded && !simResult && (
                          <div className="mt-2 ml-8 p-2 bg-slate-50 rounded-lg">
                            <div className="grid grid-cols-3 gap-2 text-[10px]">
                              <div><span className="text-slate-400">能力评估</span><p className="font-medium text-slate-700">{Math.round(s.readiness * 0.9)}/100</p></div>
                              <div><span className="text-slate-400">经验匹配</span><p className="font-medium text-slate-700">{Math.round(s.readiness * 1.05)}/100</p></div>
                              <div><span className="text-slate-400">发展潜力</span><p className="font-medium text-slate-700">{Math.round(s.readiness * 0.95)}/100</p></div>
                            </div>
                          </div>
                        )}
                      </div>
                ...
                {/* Risk Alert */}
                {pos.riskLevel === 'high' && (
                  <div className="mt-3 p-2.5 bg-error-50 rounded-lg flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4 text-error-500" />
                    <span className="text-xs text-error-700">继任梯队就绪度不足，建议加速培养计划</span>
                  </div>
                )}
              </div>
            </div>
