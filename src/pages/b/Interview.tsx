  ...
  return (
    <div className="p-8 h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-slate-900">面试监控</h1><p className="text-sm text-slate-500 mt-1">Interview Monitor · AI异步面试 · 多模态信号分析 · 实时追踪</p></div>
        <div className="flex items-center gap-3">
          <button onClick={() => setShowMatrix(!showMatrix)} className={`btn-secondary ${showMatrix ? 'ring-2 ring-primary-200' : ''}`}>
      ...
      {showMatrix && (
        <div className="mb-6 glass-card p-5 animate-in">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-800">多维评分矩阵 · {selected.candidate}</h3>
            <span className="text-xs text-slate-400">权重总和 = 1.0</span>
          </div>
          <div className="grid grid-cols-5 gap-4">
            {scoringMatrix.map(dim => {
              ...
              return (
                <div key={dim.dimension} className="p-3 bg-slate-50 rounded-xl">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-medium text-slate-700">{dim.dimension}</span>
                    <span className="text-[10px] text-slate-400">×{dim.weight}</span>
                  </div>
                  <p className="text-xl font-bold text-primary-600 mb-2">{score}</p>
                  ...
                  <div className="space-y-1">
                    {dim.subItems.map(sub => (
                      <div key={sub} className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-medium text-slate-700">{Math.round(score + (Math.random() * 8 - 4))}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
        ...
        <div className="col-span-3 glass-card p-4 overflow-y-auto">
          ...
          <div className="space-y-2">
            {interviewSessions.map(s => (
              <div key={s.id} onClick={() => setSelected(s)} className={`p-3 rounded-xl cursor-pointer transition-all ${selected.id === s.id ? 'bg-primary-50 border border-primary-200 shadow-sm' : 'hover:bg-slate-50 border border-transparent'}`}>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-slate-800">{s.candidate}</span>
                  <span className={`badge ${s.status === 'completed' ? 'badge-green' : 'badge-yellow'}`}>{s.status === 'completed' ? '已完成' : '进行中'}</span>
                </div>
                <p className="text-xs text-slate-500 mt-1">{s.position}</p>
                <div className="flex items-center gap-2 mt-2">
                  <span className="text-[10px] text-slate-400">{s.startTime}</span>
                  <span className="text-[10px] text-primary-500 font-medium">{s.dynamicQuestions}轮追问</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="col-span-5 glass-card p-6 flex flex-col min-h-0">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-semibold text-slate-800">{selected.candidate} · {selected.position}</h3>
              <p className="text-xs text-slate-500">{selected.startTime} · 时长 {selected.duration}min · {selected.dynamicQuestions}轮动态追问</p>
            </div>
            {selected.status === 'completed' && (
              <div className="flex items-center gap-4">
                <div className="text-center"><p className="text-lg font-bold text-primary-600">{selected.hardSkillScore}</p><p className="text-[10px] text-slate-400">硬技能</p></div>
                <div className="text-center"><p className="text-lg font-bold text-trust-600">{selected.softSkillScore}</p><p className="text-[10px] text-slate-400">软技能</p></div>
                <div className="text-center"><p className="text-lg font-bold text-success-600">{selected.authenticityScore}</p><p className="text-[10px] text-slate-400">真实性</p></div>
              </div>
            )}
          </div>
          <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-3 pr-2">
            {selected.transcript.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === 'candidate' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === 'ai' ? 'bg-primary-100 text-primary-600' : msg.role === 'candidate' ? 'bg-slate-200 text-slate-600' : 'bg-warning-100 text-warning-600'}`}>
                  {msg.role === 'ai' ? <Bot className="w-3.5 h-3.5" /> : msg.role === 'candidate' ? <User className="w-3.5 h-3.5" /> : <Info className="w-3.5 h-3.5" />}
                </div>
                <div className={`max-w-[75%] p-3 rounded-xl ${msg.role === 'ai' ? 'bg-primary-50 border border-primary-100' : msg.role === 'candidate' ? 'bg-slate-50 border border-slate-100' : 'bg-warning-50 border border-warning-100'}`}>
                  <p className="text-sm text-slate-700 leading-relaxed">{msg.content}</p>
                  <div className="flex items-center gap-2 mt-1.5">
                    <span className="text-[10px] text-slate-400">{msg.timestamp}</span>
                    {msg.evidenceLink && <span className="text-[10px] text-primary-500 font-mono">#{msg.evidenceLink}</span>}
                  </div>
                </div>
              </div>
            ))}
            {liveMessages.map((msg, i) => (
              <div key={`live-${i}`} className={`flex gap-3 ${msg.role === 'candidate' ? 'flex-row-reverse' : ''} animate-in`}>
                ...
                <div className={`max-w-[75%] p-3 rounded-xl ${msg.role === 'ai' ? 'bg-primary-50 border border-primary-100' : msg.role === 'candidate' ? 'bg-slate-50 border border-slate-100' : 'bg-success-50 border border-success-100'}`}>
                  <p className="text-sm text-slate-700 leading-relaxed">{msg.content}</p>
                  <span className="text-[10px] text-primary-500 mt-1 inline-block">{msg.timestamp}</span>
                </div>
              </div>
            ))}
            {isLive && <div className="flex items-center gap-2 p-2"><div className="w-2 h-2 rounded-full bg-primary-500 animate-pulse" /><span className="text-xs text-slate-400">AI正在分析回答...</span></div>}
          </div>
        </div>
        ...
        <div className="col-span-4 flex flex-col gap-4 min-h-0">
          ...
          <div className="glass-card p-4 flex-1 overflow-y-auto">
            ...
            <div className="flex items-center gap-3 mb-3 p-2 bg-slate-50 rounded-lg">
              <div className="flex items-center gap-1.5"><Mic className="w-3 h-3 text-purple-500" /><span className="text-[10px] text-slate-500">语音</span></div>
              <div className="flex items-center gap-1.5"><Video className="w-3 h-3 text-blue-500" /><span className="text-[10px] text-slate-500">视觉</span></div>
              <div className="flex items-center gap-1.5"><FileText className="w-3 h-3 text-emerald-500" /><span className="text-[10px] text-slate-500">文本</span></div>
            </div>
            <div className="space-y-2">
              {multiModalSignals.slice(0, isLive ? signalIdx : multiModalSignals.length).map((sig, i) => (
                <div key={i} className={`p-2.5 rounded-lg border transition-all animate-in ${sig.sentiment === 'positive' ? 'bg-emerald-50/50 border-emerald-100' : sig.sentiment === 'negative' ? 'bg-red-50/50 border-red-100' : 'bg-slate-50 border-slate-100'}`}>
                  <div className="flex items-center gap-2">
                    {signalIcon(sig.type)}
                    <span className="text-[10px] text-slate-400 font-mono">{sig.time}</span>
                    <div className="flex-1" />
                    <span className={`text-[10px] font-medium ${sig.confidence >= 0.9 ? 'text-emerald-600' : sig.confidence >= 0.75 ? 'text-primary-600' : 'text-amber-600'}`}>{Math.round(sig.confidence * 100)}%</span>
                  </div>
                  <p className="text-xs text-slate-700 mt-1 ml-5">{sig.signal}</p>
                </div>
              ))}
              {isLive && signalIdx < multiModalSignals.length && (
                <div className="flex items-center gap-2 p-2"><div className="w-2 h-2 rounded-full bg-purple-500 animate-pulse" /><span className="text-[10px] text-slate-400">检测中...</span></div>
              )}
            </div>
          </div>
          ...
          <div className="glass-card p-4 h-48 overflow-y-auto">
            ...
            <div className="space-y-1.5">
              {sseEvents.length === 0 && <p className="text-xs text-slate-400 py-4 text-center">点击"模拟实时面试"开始</p>}
              {sseEvents.map((ev, i) => (
                <div key={i} className="p-2 rounded-lg bg-slate-50 text-xs text-slate-600 animate-in">{ev}</div>
              ))}
            </div>
          </div>
          ...
          <div className="glass-card p-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2"><Shield className="w-3.5 h-3.5 text-trust-500" /><span className="text-xs text-slate-600">面试过程CA签名存证</span></div>
              <div className="flex items-center gap-2"><Zap className="w-3.5 h-3.5 text-primary-500" /><span className="text-xs text-slate-600">AI追问策略自动触发</span></div>
              <div className="flex items-center gap-2"><Eye className="w-3.5 h-3.5 text-purple-500" /><span className="text-xs text-slate-600">多模态信号融合分析</span></div>
              <div className="flex items-center gap-2"><AlertTriangle className="w-3.5 h-3.5 text-warning-500" /><span className="text-xs text-slate-600">异常行为实时检测</span></div>
            </div>
          </div>
        </div>
      </div>
