  ...
  return (
    <div className="p-8 h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-slate-900">人才图谱</h1><p className="text-sm text-slate-500 mt-1">Knowledge Graph · 关系穿透 · CA签名边验证 · 节点下钻</p></div>
        <div className="flex items-center gap-3">
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="搜索节点..." className="pl-9 pr-3 py-1.5 text-sm border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-primary-200 focus:border-primary-300 w-44" />
          </div>
          <label className="flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer">
            <input type="checkbox" checked={showEdgeLabels} onChange={e => setShowEdgeLabels(e.target.checked)} className="rounded border-slate-300" />
            边标签
          </label>
      ...
      <div className="flex-1 flex gap-6 min-h-0">
        <div className="flex-1 glass-card overflow-hidden relative">
          ...
          {/* Legend */}
          <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm rounded-xl p-3 border border-slate-200 shadow-sm">
            <p className="text-[10px] font-semibold text-slate-600 mb-2">图例</p>
            <div className="space-y-1.5">
              {Object.entries(typeLabels).map(([k, v]) => (<div key={k} className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ background: typeColors[k] }} /><span className="text-[10px] text-slate-600">{v}</span></div>))}
              <div className="border-t border-slate-100 pt-1.5 mt-1.5">
                <div className="flex items-center gap-2"><div className="w-6 h-0.5 bg-emerald-500" /><span className="text-[10px] text-slate-600">CA签名边</span></div>
                <div className="flex items-center gap-2 mt-1"><div className="w-6 h-0.5 bg-slate-300 border-dashed border-t" /><span className="text-[10px] text-slate-600">未签名边</span></div>
              </div>
              <div className="flex items-center gap-2"><Shield className="w-3 h-3 text-trust-500" /><span className="text-[10px] text-slate-600">CA验证节点</span></div>
            </div>
          </div>
          {!selected && <div className="absolute top-4 left-4 bg-white/80 backdrop-blur-sm rounded-lg px-3 py-2 border border-slate-100"><p className="text-xs text-slate-500">点击节点查看详情与关联关系</p></div>}
        </div>
        {/* Detail Panel */}
        {selected && (
          <div className="w-80 glass-card p-5 flex flex-col overflow-y-auto animate-in">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-slate-800">节点详情</h3>
            ...
            <div className="flex items-center gap-3 mb-4 p-3 rounded-xl bg-slate-50">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{typeLabels[selected.type]} · {selected.connections}个关联</p>
              </div>
            </div>
            {selected.caVerified && (
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                {getConnectedNodes(selected.id).map(cn => (
                  <div key={cn.id} onClick={() => setSelected(cn)} className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-slate-700 truncate">{cn.name}</p>
                      <p className="text-[10px] text-slate-400">{typeLabels[cn.type]}</p>
                    </div>
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between text-xs p-2 rounded-lg bg-slate-50">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-slate-600">{edgeTypeLabels[e.type] || e.type} → {other?.name}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-slate-400">权重 {e.weight}</span>
