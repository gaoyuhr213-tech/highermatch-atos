...
const categoryConfig: Record<string, { label: string; icon: typeof Building2; color: string }> = {
  work_content: { label: '工作内容', icon: Building2, color: 'text-primary-600 bg-primary-50' },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">岗位真实答疑</h1>
          <p className="text-sm text-slate-500 mt-1">CA认证企业发布真实工作内容 · 消除信息差 · 提升到面率</p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-4 space-y-3">
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-card p-4">
            <h3 className="text-sm font-semibold text-slate-700 mb-3">认证岗位列表</h3>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs font-semibold text-primary-600 mt-1">{job.salary}</p>
                    </div>
                    <span className="text-[10px] text-slate-400">{job.qaCount}条答疑</span>
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-4 mt-2">
                  <span className="text-sm text-slate-600">{selectedJob.company}</span>
                  <span className="text-sm text-slate-400">|</span>
                  <span className="text-sm text-slate-600">{selectedJob.location}</span>
                  <span className="text-sm text-slate-400">|</span>
                  <span className="text-sm font-semibold text-primary-600">{selectedJob.salary}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-800">{selectedJob.qaCount}</p>
                  <p className="text-[10px] text-slate-400">条真实答疑</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {selectedJob.items.map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-white rounded-xl border border-slate-200/80 shadow-card p-5 hover:shadow-card-hover transition-shadow"
                  >
                    {/* 问题 */}
                    ...
                    {/* 回答 */}
                    <div className="mt-4 ml-9 p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <p className="text-sm text-slate-700 leading-relaxed">{item.answer}</p>
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100">
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-1.5">
                            <div className="w-5 h-5 rounded-full bg-primary-100 flex items-center justify-center">
                              <span className="text-[9px] font-bold text-primary-700">{item.author[0]}</span>
                            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-card p-4">
            <div className="flex items-center gap-3">
              <input
                ...
                className="flex-1 px-4 py-2.5 bg-slate-50 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 transition-all"
              />
              <button className="px-4 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-medium hover:bg-primary-700 transition-colors flex items-center gap-2 shadow-sm">
                <Send className="w-4 h-4" />
                <span>提问</span>
              </button>
            </div>
            <p className="text-[10px] text-slate-400 mt-2 ml-1">提问内容经审核后发布 · 仅CA认证企业可回答 · 实名理性社区</p>
          </div>
        </div>
      </div>
    </div>
