  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">同行互助圈</h1>
          <p className="text-sm text-slate-500 mt-1">CA认证HR实名交流 · 招聘经验共享 · 数据驱动决策</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-primary-50 rounded-lg border border-primary-100">
            <Shield className="w-3.5 h-3.5 text-primary-600" />
            <span className="text-xs font-semibold text-primary-700">实名理性社区</span>
          </div>
          {FEATURE_FLAG_ANONYMOUS_EMOTION && (
            <div className="px-3 py-1.5 bg-slate-100 rounded-lg">
              <span className="text-xs text-slate-400">匿名模式(已关闭)</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 transition-all"
          />
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredPosts.map((post, index) => (
          <motion.div
            ...
            className="bg-white rounded-xl border border-slate-200/80 shadow-card p-5 hover:shadow-card-hover transition-all cursor-pointer group"
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-slate-400">{post.company}</span>
                ...
                {/* Content preview */}
                <p className="text-sm text-slate-600 mt-2 line-clamp-2 leading-relaxed">{post.content}</p>

                {/* Tags */}
                <div className="flex items-center gap-2 mt-3">
                  {post.tags.map(tag => (
                    <span key={tag} className="px-2 py-0.5 bg-slate-50 rounded-md text-[10px] font-medium text-slate-500 border border-slate-100">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-50">
                  ...
                  <span className="flex items-center gap-1.5 text-xs text-slate-500 ml-auto">
                    <span className="px-2 py-0.5 bg-slate-50 rounded text-[10px] font-medium">{post.category}</span>
                  </span>
                </div>
              </div>
            </div>
